...
- Shortcuts: ⌘/Ctrl+Z undo · ⌘/Ctrl+Shift+Z (or Ctrl+Y) redo · ⌘/Ctrl+Shift+V paste copied property values · Delete/Backspace removes selection (Mac uses ⌘, Windows/Linux use Ctrl)
...
- Inspector: left rail (Content / Background / Slide Template / Export) expands a tool panel to the right; canvas center; selected-item Properties on the right in accordion sections (click header to expand/collapse; one open at a time). Selecting a layer on the canvas opens **Content**, switches to the matching Phone/Text/Clipart/Lens tab, and highlights that row. Same order for every component: **type content** (Screen / Content / Attach / Image) → **Position** → **Transform** → **Style** (when applicable) → **Shadow** → **Edges**. Thin `.range-thin` sliders with purple fill; actions stay pinned at the bottom. **Copy** on a section header or field copies values into an in-app property clipboard; **Paste props** (or ⌘/Ctrl+Shift+V) applies them to another component (size metrics only paste across matching kinds).
