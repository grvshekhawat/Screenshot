...
function normalizeLayout(raw: unknown): AnalyzedLayout {
  ...
  const devices = devicesIn.slice(0, 2).map((d) => {
    ...
    // If screen equals full image but deviceBox is smaller, nest screen inside device.
  ...
  const cliparts = clipartsIn.slice(0, 6).map((c) => {
...
function mimeFromPath(path: string, fallback: string): string {
  if (path.endsWith(".png") || fallback.includes("png")) return "image/png"
  if (path.endsWith(".webp") || fallback.includes("webp")) return "image/webp"
  return "image/jpeg"
}
...
function fallbackLayout(): AnalyzedLayout {
  return {
    background: {
      type: "solid",
...
serve(async (req) => {
  ...
  for (const assetId of assetIds) {
    ...
    const mime = mimeFromPath(path, file.type || "image/jpeg")
    try {
      const layout = await analyzeOne(openaiKey, buffer, mime)
      layouts.push(layout)
    } catch (err) {
